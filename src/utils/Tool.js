  ...
  function httpEnd() {
    if (xhr.readyState == 4) {
      ...
      if (
        ...
        (setting.dataType === "json" && /^(\{|\[)([\s\S])*?(\]|\})$/.test(response))
      ) {
      ...
      if (xhr.status == 200) {
        ...
        setTimeout(() => {
          //异步错误处理，保证请求状态在错误处理逻辑之前改变，否则无法保证正确处理错误状态
          if (xhr.status === 0) {
...
Tool.getLocation = function(address, success, error) {
  $.ajax({
    url:
      ...
      address +
      "&output=json&ak=V5qLiGqxsGZZpRt8nqUERSpA2A18MLjk",
    type: "GET",
    dataType: "jsonp",
    jsonp: "callback",
    jsonpCallback: "jscb",
